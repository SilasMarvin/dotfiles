...
main :: IO ()
main = do
  xmonad . fullscreenSupport . docks . ewmh . ewmhFullscreen . pagerHints $ myConfig
  ...
  where
    ...
    layoutRelated =
      [ ((myModMask, xK_n), sendMessage NextLayout),
        ...
        ((myModMask .|. shiftMask, xK_f), windows focusMaster),
...
myLayoutModifiers :: MyTogglableLayouts Window -> (MyLayoutModifiers MyTogglableLayouts) Window
myLayoutModifiers =
  ...
  where
    ...
    --(windowBorder, screenBorder, edgeGap) = case win of
    --  FULL -> (Border {top = 0, bottom = 0, right = 0, left = 0}, Border {top = 0, bottom = 0, right = 0, left = 0}, 0)
    --  _ -> (Border {top = 5, bottom = 5, right = 5, left = 5}, Border {top = 5, bottom = 5, right = 5, left = 5}, 5) 
